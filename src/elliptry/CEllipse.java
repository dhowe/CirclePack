...
public class CEllipse {
	...
	public CEllipse(int x, int y, int width, int height) { // diameters
	...
	public CEllipse(float x, float y, float width, float height) {

		this(Math.round(x), Math.round(y), Math.round(width), Math.round(height));
	}

	public CEllipse(double x, double y, double width, double height) {
	...
	public static void main(String[] args) {
		...
		System.out.println(new CEllipse(0, 0, 12, 4).area() / Math.PI); // ~12
	}
